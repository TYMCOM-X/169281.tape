    ...
    Originator:
    Carl A Baltrunas ________________________________            Date:________
    ...
    Craig Fletcher __________________________________            Date:________
    Manager
...
System Profile and Performance Analysis                                Page  1
...
1.  Overview
    ...
    This utility requires Osiris to make available the following information:
        ...
        B.  Error history information

            Error statistics must be available from each subsystem specifying
            each type of failure (hard or soft) and how many failures.  Some
            subsystems may require multiple sets of counters depending on what
            type of object describes it.  The system administrator needs this in
            order to determine that a hardware or software subsystem requires
            service or replacement.  Information need only be kept since the
            installation or creation of each object (for that object).

        C.  Usage history information
	    ...
	    job or session), for all usage since system reload and for all
	    usage since creation of the object.

        D.  Wait queue information
...
System Profile and Performance Analysis                                Page  2
...
2.  Requirements
    ...
    Required:  Ability to read error history information including failure
	types, counts and acceptable thresholds for each hardware and software
	subsystem / object.

	Each type of failure a subsystem (device or object) may sustain must be
        tracked.  To be fault-tolerant, each type needs to be categorized as
        fatal or recoverable.  Fatal failures require immediate attention and
        must indicate that a problem exists immediately via the SSB or some
        other mechanism.  Recoverable failures must be compared against a user
        specified threshold so that appropriate escalation is possible when the
        threshold is reached.

        Error statistics could be incorportated into the system configuration
        report or broken out into separate reports so that problem areas may
        be identified and maintenance or replacement of hardware and software
        components may be scheduled.
    ...
    Required:  Storage space in the object directory entries and the memory-
        ...
        Osiris subsystem are needed for any type of performance analysis.  A
        space should be allocated for three levels of information:  for the
        current session, since system startup and since object installation or
        creation.
    ...
    Required:  Ability to read access counts and data bytes transferred to and
        ...
        while reading it.
System Profile and Performance Analysis                                Page  3
...
System Profile and Performance Analysis                                Page  4
...
4.  Requirement for Other Projects
    ...
    Implementation of the above requirements permits the data to be utilized
    in many applications ranging from statistic gathering to system usage
    accounting.
