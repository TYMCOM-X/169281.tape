...
C
     ...
     *FILEX,LFILID,VAL370
      INTEGER LOGSTR(5),ACTCMD(3),KEY(4),WHONAM(3)
...
C
      ...
      OPEN (UNIT=20,FILE='(KHANHQN)ACTCOD.HST',ACCESS='SEQIN',ERR=1020)
      NSYS= 1
...
170   FORMAT(/T2,A5,' - Building circuit to Lud Update System.')
      CLOSE (UNIT=TOUT)
      CALL BLDCIR('VALDEV:35;','GO LUSP$',PORT,ERRCOD)
...
190   LUDDWN= .FALSE.
      ...
      GO TO 850
200   IF (.NOT.ACTDWN) GO TO 210
      CALL RENAM(LFILID,FILEID,IERR)
      ACTDWN= .FALSE.
210   CALL OPNFIL(20,FILEID,'APPEND',$210)
      CALL S1BYTE(PORT,1)
      CALL G1BYTE(PORT,RTNCOD,$240)
      IF (RTNCOD.NE.1) GO TO 300
220   CALL GUNAME(PORT,UNAME,12,$300,$260)
      CALL G3BYTE(PORT,UUN,$260)
      WRITE(20,230) UNAME,UUN
230   FORMAT(' 1',2A5,A2,O6,12X)
      GO TO 220
240   OPEN (UNIT=TOUT,FILE=TOUTFL,ACCESS='APPEND')
      CALL TIME(CTIME)
      WRITE(TOUT,250) CTIME
250   FORMAT(X,A5,' - Circuit zapped during get all delete names.')
      CLOSE (UNIT=TOUT)
      GO TO 400
260   OPEN (UNIT=TOUT,FILE=TOUTFL,ACCESS='APPEND')
      CALL TIME(CTIME)
      WRITE(TOUT,270) CTIME,FILEID
270   FORMAT(X,A5,' - Circuit zapped during copy LUDUPD.DEL to ',A10)
      CLOSE (UNIT=TOUT)
      GO TO 400
300   CALL S1BYTE(PORT,2)
      CALL G1BYTE(PORT,RTNCOD,$330)
      IF (RTNCOD.NE.1) GO TO 400
310   CALL GUNAME(PORT,UNAME,12,$400,$350)
      CALL G3BYTE(PORT,UUN,$350)
      CALL GUNAME(PORT,NEWNAM,12,$400,$350)
      WRITE(20,320) UNAME,UUN,NEWNAM
320   FORMAT(' 3',2A5,A2,O6,2A5,A2)
      GO TO 310
330   OPEN (UNIT=TOUT,FILE=TOUTFL,ACCESS='APPEND')
      CALL TIME(CTIME)
      WRITE(TOUT,340) CTIME
340   FORMAT(X,A5,' - Circuit zapped during get all change names.')
      CLOSE (UNIT=TOUT)
      GO TO 400
350   OPEN (UNIT=TOUT,FILE=TOUTFL,ACCESS='APPEND')
      CALL TIME(CTIME)
      WRITE(TOUT,360) CTIME,FILEID
360   FORMAT(X,A5,' - Circuit zapped during copy LUDUPD.CHG to ',A10)
      CLOSE (UNIT=TOUT)
400   CLOSE (UNIT=20)
410   NREC= 0
...
550   CALL HSTART(0)
      ...
      LUDDWN= .FALSE.
560   CALL MAKFIL(FILEID,FILEX,'. XX')
...
650   CALL G2BYTE(PORT,HOST,$590)
      ...
      CALL FWRITE(ERRFIL,HOST,$650)
660   CALL FWRITE(PROFIL,HOST,$650)
670   CALL FWRITE(IBMFIL,HOST,$650)
C
...
780   CLOSE (UNIT=20)
      CALL ZAPC(PORT)
785   CALL MAKFIL(FILEID,PROFIL,'.PDP')
...
795   CALL MAKFIL(FILEID,SRTFIL,'. SS')
      CALL SORT(PROFIL,SRTFIL,KEY,MERR,PERR)
...
C
850   PHOST= 0
      CALL MAKFIL(FILEID,HSTDWN,'. DD')
      CALL OPNFIL(20,SRTFIL,'SEQIN',$850)
