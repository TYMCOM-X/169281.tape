        IMPLICIT INTEGER (A-Z)
        INTEGER UNAME(3),NNAME(3)
        CALL STROUT('BUILDING CIRCUIT TO LUD UPDATE SYSTEM$',-1,1,1)
        CALL BLDCIR('KHANHQN:55;','LUSP',PORT,HOSTER,SUPER)
        IF ((HOSTER.NE.0).OR.(SUPER.NE.0)) GO TO 999
        CALL HSHAKE(PORT,FLAG)
        IF (FLAG.NE.0) GO TO 998
        CALL STROUT('CIRCUIT ESTABLISHED$',-1,1,1)
5       READ (21,10,END=900)CODE,UNAME,UUN,NNAME
10      FORMAT(I1,X,2A5,A2,X,O8,2A5,A2)
        CALL S1BYTE(PORT,CODE)
        CALL SUNAME(PORT,UNAME)
        CALL S3BYTE(PORT,UUN)
        CALL SUNAME(PORT,NNAME)
        CALL G1BYTE(PORT,CODE)
        TYPE 30,CODE
30      FORMAT(' RETURN CODE: ',I5)
        GO TO 5
900     CLOSE (UNIT=21)
        CALL ZAPC(PORT)
        STOP
998     TYPE 100
100     FORMAT(' FAIL HAND SHAKE')
        STOP
999     TYPE 110
110     FORMAT(' CANNOT BUILD CIRCUIT')
        STOP    
        END
